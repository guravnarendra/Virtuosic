...
function initCountdown() {
  if (window.__VIRTUOSIC_COUNTDOWN_INTERVAL) {
          ...
          const slowInterval = setInterval(() => {
            ...
            if (slowStep < slowFlips) {
              ...
              if (id === 'seconds') {
                setTimeout(() => {
                  tick();
                  window.__VIRTUOSIC_COUNTDOWN_INTERVAL = setInterval(tick, 1000);
                }, 600);
              }
            }
...
function expandToPage(url, originEl) {
  ...
  setTimeout(() => {
    if (typeof window.__NEXT_NAVIGATE === 'function') {
      window.__NEXT_NAVIGATE(url);
      return;
    }
...
window.expandToPage = expandToPage;
window.__VIRTUOSIC_INIT = initVirtuosicPage;
